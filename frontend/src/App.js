...
export default function App() {
  ...
  useEffect(() => {
    ...
    apiCall('/api/auth/profile')
      .then(res => { if (res?.success && res?.user) setUser(res.user); else localStorage.removeItem('mk_token'); })
  ...
  const navigate = id => {
    setPage(id);
    const validTabs = ['home','bids','disawar','jackpot','wallet','profile','game','txns','support'];
