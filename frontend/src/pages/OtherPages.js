...
// ── DEPOSIT MODAL ──────────────────────────────────────────────────────────────
export function DepositModal({ onClose, apiCall, onSuccess }) {  const [step, setStep] = useState(1); // 1=amount, 2=payment
  ...
  useEffect(() => {
    if (apiCall) {
      apiCall('/api/admin/settings').then(res => {
        if (res?.success && res?.settings?.upi_id) {
          const id = res.settings.upi_id;
          setUpiId(id);
          // Generate QR using public API
          setQrUrl(`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=upi://pay?pa=${encodeURIComponent(id)}&pn=MatkaKing&cu=INR`);
        }
      }).catch(() => {});
    }
  }, [apiCall]);

  // Step 1 → Step 2 (show payment screen)
  const handleNext = () => {
  ...
  const sheetStyle = {
    ...
    padding: '0 0 32px', boxShadow: '0 -8px 40px rgba(0,0,0,0.2)'
  };
    ...
    <div style={overlayStyle} onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div style={sheetStyle}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 20px 16px' }}>
          <div style={{ fontSize: 18, fontWeight: 900, color: '#0d1f40' }}>
            {step === 1 ? '💰 Add Money' : '📱 Pay ₹' + amount}
          </div>
        ...
        {step === 1 && (
          <div style={{ padding: '0 20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 20 }}>
              {presets.map(p => (
                <button key={p}
                  ...
                  style={{
                    padding: '12px 0', borderRadius: 12, cursor: 'pointer', fontWeight: 800, fontSize: 14,
                    background: amount === String(p) ? 'linear-gradient(135deg,#1e4fa0,#2a6dd9)' : '#eef2f7',
                    color: amount === String(p) ? '#fff' : '#0d1f40',
                    border: amount === String(p) ? 'none' : '1.5px solid #d0daea',
                    transition: 'all 0.2s'
                  }}>
            ...
            <input
              style={{ ...B.input, fontSize: 20, fontWeight: 900, textAlign: 'center' }}
            ...
            {msg.text && (
              <div style={{ background: msg.type === 'ok' ? '#e8f5e9' : '#fdecea', border: `1px solid ${msg.type === 'ok' ? '#a5d6a7' : '#f5c6cb'}`, borderRadius: 10, padding: '12px', marginBottom: 16, color: msg.type === 'ok' ? '#1e8a3c' : '#c0392b', fontSize: 13, fontWeight: 700 }}>{msg.text}</div>
            )}

            <button onClick={handleNext} style={B.btn}>PROCEED TO PAY →</button>

            <div style={{ textAlign: 'center', marginTop: 12, fontSize: 11, color: '#8a9bb5' }}>
              💡 Deposit approve hone mein 15-30 min lagte hain
            </div>
          </div>
        )}
          ...
          <div style={{ padding: '0 20px' }}>
            ...
            <button
              ...
              style={{
                ...
                background: 'linear-gradient(135deg, #00b09b, #1e8a3c)',
                color: '#fff', border: 'none', borderRadius: 14,
                fontSize: 16, fontWeight: 900, cursor: 'pointer',
                display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10,
                boxShadow: '0 6px 20px rgba(0,176,155,0.4)', letterSpacing: 0.5
              }}>
            ...
            {qrUrl && (
              <div style={{ textAlign: 'center', marginBottom: 16 }}>
                ...
                {upiId && (
                  <div style={{ marginTop: 10, fontSize: 13, color: '#8a9bb5' }}>
                    UPI ID: <span style={{ fontWeight: 900, color: '#0d1f40' }}>{upiId}</span>
                  </div>
                )}
              </div>
            )}
            ...
            <button onClick={handleSubmitUTR} disabled={loading} style={{ ...B.btn, opacity: loading ? 0.6 : 1 }}>
              {loading ? '⏳ Submitting...' : '✅ SUBMIT UTR'}
            </button>

            <button onClick={() => { setStep(1); setMsg({ type: '', text: '' }); }}
              style={{ width: '100%', marginTop: 10, padding: '14px', background: 'transparent', border: '1.5px solid #d0daea', borderRadius: 14, color: '#8a9bb5', fontWeight: 800, cursor: 'pointer', fontSize: 14 }}>
              ← Change Amount
            </button>
          </div>
        )}
  ...
  return (
    <div style={B.page}>
      <SubHeader
        ...
        rightBtn={
          <button onClick={fetchTxns} style={{ background: '#eef2f7', border: '1px solid #d0daea', color: '#2a6dd9', padding: '8px 12px', borderRadius: 10, cursor: 'pointer', fontSize: 14, fontWeight: 700 }}>🔄</button>
        }
      />
...
export function WalletPage({ wallet, onAdd, onWith, user, navigate, apiCall }) {
  ...
  // onAdd ko override karke apna modal dikhao
